...
class WebPartnerRepository extends EntityRepository
{
    public function getActiveWebPartners(?DateTime $referenceDateTime = null, ?int $limit = null): Collection
    {
        ...
        $queryBuilder->where(':ref BETWEEN p.startDateTime AND p.endDateTime')->setParameter('ref', $referenceDateTime);
        $queryBuilder->orWhere('p.startDateTime IS NULL AND :ref < p.endDateTime')->orWhere('p.endDateTime IS NULL AND :ref > p.startDateTime');
        $queryBuilder->orderBy('priority', 'DESC')->addOrderBy('id', 'ASC');
        if (null !== $limit) {
